...
class CarSimulator:
    def __init__(self, grid_size=600, cell_size=30):
        ...
        self.canvas = tk.Canvas(
            self.root, width=grid_size, height=grid_size, bg="white"
        )
        ...
        self.run_algo_button = tk.Button(
            self.control_frame, text="Run Algorithm", command=self.run_algo
        )
        self.run_algo_button.pack(side=tk.BOTTOM, anchor="se", padx=10, pady=10)

        # Add the "Reset All" button
        self.reset_button = tk.Button(
            self.control_frame, text="Reset All", command=self.reset_simulation
        )
    ...
    def draw_grid(self):
        for i in range(0, self.grid_size, self.cell_size):
            self.canvas.create_line(
                [(i, 0), (i, self.grid_size)], tag="grid_line", fill="black"
            )
            self.canvas.create_line(
                [(0, i), (self.grid_size, i)], tag="grid_line", fill="black"
            )
    ...
    def update_car_position(self, x, y, direction):
        ...
        self.draw_car(
            x, y, direction
        )  # Now calling the updated draw_car function with bottom-left coordinates
    ...
    def draw_path(self, prev_x, prev_y, new_x, new_y):
        ...
        self.canvas.create_line(
            ...
            + 0.5 * self.cell_size,  # Center of the previous car position
            self.grid_size - prev_y * self.cell_size - 0.5 * self.cell_size,
            new_x * self.cell_size
            + 0.5 * self.cell_size,  # Center of the new car position
            self.grid_size - new_y * self.cell_size - 0.5 * self.cell_size,
            fill="blue",
            width=2,
            tags="path",  # Add a tag for the path
        )
    ...
    def simulate_camera_action(self, direction):
        ...
        self.canvas.itemconfig(
            car_blocks[1], fill="red"
        )  # Camera indicates picture taken
        self.root.update()
    ...
    def update_obstacle_image(self, key, img_id, direction_var):
        ...
        x1, y1 = (
            botleft_x * self.cell_size,
            self.grid_size - (botleft_y + OBS_DIM) * self.cell_size,
        )
        x2, y2 = (
            botleft_x + OBS_DIM
        ) * self.cell_size, self.grid_size - botleft_y * self.cell_size
    ...
    def draw_obstacle(self, botleft_x, botleft_y, image_direction):
        ...
        rect_id = self.canvas.create_rectangle(
            x1, y1, x2, y2, fill="blue", tags="obstacle"
        )
        ...
        img_id = self.canvas.create_rectangle(
            img_x1, img_y1, img_x2, img_y2, fill="#C71585", tags="obstacle_image"
        )
